...
def login_page():
    ...
    # st.markdown("<h1 style='text-align: center; color: #FF4B4B; margin-right: 13px; margin-top: 70px; '>User Login</h1>", unsafe_allow_html=True)
    st.markdown("")
    st.markdown("")
    st.markdown("")
    st.markdown("")
...
def orders_management_page():
    ...
    if page=='Analysis':
        ...
        query = """
            SELECT 
                (SELECT COALESCE(SUM(shipping_price), 0) FROM orders) AS total_completed_shipping_price,
                (SELECT COALESCE(SUM(shipping_price), 0) FROM returned_orders) AS total_returned_shipping_price,
                (SELECT COALESCE(SUM(shipping_price), 0) FROM shipping) AS total_shipping_problems_price,
                (SELECT COALESCE(SUM(shipping_price), 0) FROM orders) + 
                (SELECT COALESCE(SUM(shipping_price), 0) FROM returned_orders) + 
                (SELECT COALESCE(SUM(shipping_price), 0) FROM shipping) AS total_shipping_price;
        """
        cursor.execute(query)
        result = cursor.fetchone()
        total_shipping_price = result[3]
        total_query = """
                SELECT 
        ...
        total_profit=total_prices-total_shipping_price
        ...
        total_shipping_returned_price = cursor.fetchone()[0]
        ...
        if selected_3=="Add Order":
            ...
            # Define unit prices for each product type
            product_prices = {
                "hoodie": 850,
                "quarter zipper": 800,
                "default": 750  
            ...
            # Dictionary to store the total price for each product type
            total_product_prices = {}

            for order in data:
                order_number, products_str, total_shipping_price, total_order_price = order
                if not products_str:
                    continue  # Skip if the products column is empty or null
                
                # Parse the products into a dictionary
                products_dict = parse_products(products_str)
                total_quantity = sum(products_dict.values())

                if len(products_dict) == 1:  # If only one product type exists in the order
                    product_type = list(products_dict.keys())[0]
                    # Assign the total order price directly to this product type
                    if product_type in total_product_prices:
                        total_product_prices[product_type] += total_order_price
                    else:
                        total_product_prices[product_type] = total_order_price
                else:
                    # If multiple product types, calculate the price based on unit prices
                    for product_type, quantity in products_dict.items():
            ...
            # Initialize a dictionary to store total counts for each product type
            total_counts = {}

            # Process each row
            for row in data:
                products_str = row[0]  # Assuming the 'products' column is the first column in the result
                if not products_str:
                    continue  # Skip if products string is empty or null

                # Split the string into individual product entries
                products_list = products_str.split(',')

                for product_entry in products_list:
                    # Split product_entry into product type and quantity
                    product_type, quantity = product_entry.split(':')
                    product_type = product_type.strip().lower()  # Clean product type (e.g., remove spaces)
        ...
        if selected_1=="Add Order":
            ...
            n_products = custom_number_input("Number Of Products", min_value=0,step=1)
            order_price = custom_number_input("Order Price", min_value=0,step=1)
            order_date = st.date_input("Order Date")
            if status=="Go Only":
                shipping_price = custom_number_input("Shipping Price", min_value=0,step=1)
                    ...
                    if not customer_name.strip():
                        ...
                        if existing_order or existing_order_1 or existing_order_2:
                            ...
                            if customer:
                                ...
                                customer_id = cursor.fetchone()[0]
                            cursor.execute(
                                "INSERT INTO returned_orders (customer_id, ship_company, region, order_number,reason,hoodies,order_price,shipping_price,status,order_date) VALUES (%s, %s, %s, %s,%s,%s,%s,%s,%s,%s)",
                                (customer_id, ship_company, region, order_number,reason,n_products,order_price,shipping_price,status,order_date)
                            )
            ...
            if search_order_number:
                ...
                cursor.execute(
                    """
                    SELECT o.order_number, c.customer_name, c.customer_phone_1, c.customer_phone_2,
                        c.email, o.ship_company, o.region,o.reason,o.hoodies,o.order_price,o.shipping_price,o.order_date,o.status
                    FROM returned_orders o
                    INNER JOIN customers c ON o.customer_id = c.customer_id
                    WHERE o.order_number = %s
                    """,
                    ...
                    with st.form("update_order_form"):
                        ...
                        if update_submit:
                                ...
                                cursor.execute(
                                    ...
                                    SET ship_company = %s, region = %s,reason=%s,hoodies=%s,order_price=%s,shipping_price=%s,order_date=%s,status=%s
                                    WHERE order_number = %s
                                    """,
                                    (new_ship_company, new_region, new_reason, new_number_of_hoodies, new_price, new_shipping_price, new_date, new_status, search_order_number)
                                )
            ...
            if st.button("Add Order"):
                if not customer_name.strip():
                    ...
                    if customer:
                            ...
                            customer_id = cursor.fetchone()[0]
                    cursor.execute(
                            "INSERT INTO shipping (customer_id, ship_company, region, order_number, status, shipping_price, hoodies,reason) VALUES (%s, %s, %s, %s, %s, %s, %s,%s)",
                            (customer_id, ship_company, region, order_number, status, shipping_price, n_products,reason)
                        )
